...
def init_guess(setting, data, dataset_obj, frames_seq=1, use_torso=False, **kwargs):
    ...
    if type(setting['scale_child']) is list:
        child_scale = setting['scale_child'][0] if setting['scale_child'][0] <= 1 else 0.4545
    else:
        child_scale = setting['scale_child'] if setting['scale_child'] <= 1 else 0.4545
    ...
    #scale_array = [fixed_scale, fixed_scale/2.2]
    scale_array = [fixed_scale, child_scale]
    ...
    for idx in range(dataset_obj.num_people):
        ...
        init_s = torch.tensor(scale_array[idx], dtype=dtype)
    ...
    for idx in range(dataset_obj.num_people):
        ...
        if est_scale:
            init_s = torch.tensor(np.array(scale), dtype=dtype)
            #init_s = torch.tensor(scale, dtype=dtype)
        else:
            init_s = torch.tensor(np.array(scale_array[idx]), dtype=dtype)
        init_t = torch.tensor(np.array(translations), dtype=dtype)
        init_r = torch.tensor(np.array(rotations), dtype=dtype)
...
def fix_params(setting, scale=None, shape=None):
    ...
    for m_idx, model in enumerate(models):
        ...
        if scale is not None:
            init_s = torch.tensor(model.scale, dtype=dtype)
