...
def project_to_img(joints, verts, faces, gt_joints, camera, image_path, img_folder, save_images, viz=False, path=None):
    keypoint_color_pred = [(102, 255, 102), (102, 102, 255)]
    keypoint_color_alpha = [(0, 77, 0), (0, 0, 77)]
    exp = 1
    if len(verts) < 1:
        return
    if False:
        ...
        for v, cam in enumerate(camera):
            ...
            n_frame = str(fn).split('.png')[0]
            img = cv2.imread(os.path.join(img_folder, img_dir))
            for det_idx, (joint, vert, gt_joint) in enumerate(zip(joints, verts, gt_joints[v])):
                pers_reproj_output_string = {
                    'image_id': '00000',
                    'alpha_joints': [],
                    'pred_reproj': []
                }
                pers_reproj_output_string["image_id"] = n_frame
                pers_reproj_output_string["alpha_joints"] = gt_joint.tolist()

                if joint is None or vert is None:
                    continue
                d2j = cam(joint.unsqueeze(0)).detach().cpu().numpy().astype(np.float32)
                vert = cam(vert.unsqueeze(0)).detach().cpu().numpy().astype(np.int32)
                pred_joint_reroj = []
                for joint_id, (pred_point, alpha_point) in enumerate(zip(d2j[0], gt_joint)):
                    proj_pred_joints_rerror = [0,0,0]
                    mean = 0
                    for coord_id in range(len(pred_point)):
                        proj_pred_joints_rerror[coord_id] = float(pred_point[coord_id])
                        mean += abs(pred_point[coord_id] - alpha_point[coord_id]) ** 2
                    proj_pred_joints_rerror[2] = float(math.sqrt(mean))

                    pred_joint_reroj.append(proj_pred_joints_rerror)

                pers_reproj_output_string["pred_reproj"] = pred_joint_reroj

                frame_reprojet_json[f'det_{det_idx}'] = pers_reproj_output_string
                ...
                if save_images:
                    ...
                    for p in d2j[0]:
                        cv2.circle(img, (int(p[0]),int(p[1])), 3, keypoint_color_pred[det_idx], 5)
                    if gt_joint is not None:
                        for p in gt_joint:
                            cv2.circle(img, (int(p[0]),int(p[1])), 3, keypoint_color_alpha[det_idx], 5)

            reproj_out_folder = os.path.join(os.path.dirname(path), 'reprojection', serial, cam_)
            if not os.path.exists(reproj_out_folder):
                os.makedirs(reproj_out_folder)
            #Write JSON
            with open(f'{reproj_out_folder}/{n_frame}_Reprojection.json', 'w') as f:
                json.dump(frame_reprojet_json, f)
            f.close()
...
def save_results(setting, data, result, dataset_obj,
    ...
    for i, name in enumerate(names):
        ...
        if save_images:
            ...
            project_to_img(joints, meshes, faces, keyp_p, camera, img_p, dataset_obj.img_folder, save_images_, viz=True, path=setting['img_folder'])
...
def IOU(A, B):
    ...
    H = np.float64(min(A[1][1], B[1][1])) - np.float64(max(A[0][1], B[0][1])) 
    W = np.float64(min(A[1][0], B[1][0])) - np.float64(max(A[0][0], B[0][0])) 
    if W <= 0 or H <= 0:
        return 0
    SA = np.float64((A[1][1] - A[0][1])) * np.float64((A[1][0] - A[0][0])) 
    SB = np.float64((B[1][1] - B[0][1])) * np.float64((B[1][0] - B[0][0])) 
    cross = W * H
