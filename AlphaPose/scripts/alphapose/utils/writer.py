...
class DataWriter():
    def __init__(self, cfg, opt, save_video=False,
        ...
        if opt["pose_flow"]:
            ...
            self.pose_flow_wrapper = PoseFlowWrapper(save_path=os.path.join(opt["outdir"], 'poseflow'))
    ...
    def update(self):
        ...
        while True:
            ...
            if orig_img is None:
                ...
                if self.save_video:
                    stream.release()
                write_json(final_result, self.opt["outdir"], form=self.opt["format"], for_eval=self.opt["eval"], outputfile=(self.opt["det_dir"] + '_results.json'))
            ...
            if boxes is None or len(boxes) == 0:
                ...
                if self.opt["save_img"] or self.opt["save_video"] or self.opt["vis"]:
    ...
    def write_image(self, img, im_name, stream=None):
        ...
        if self.opt["save_img"]:
            cv2.imwrite(os.path.join(self.opt["outdir"], 'vis', im_name), img)
        if self.save_video:
            stream.write(img)
