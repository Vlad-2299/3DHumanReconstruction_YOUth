...
def order_drop_view_info(drop_view_in_frames):
    ...
    while not is_range_buffer_empty(drop_view_in_frames_comp):
        ...
        for view_idx, view_ranges in enumerate(drop_view_in_frames_comp.items()): #[key_view][[range_list]]
            #print(view_idx, view_ranges)
            if view_ranges[1]:
                for range_idx, range in enumerate(view_ranges[1]):
                    if range[0] < initial:
                        initial = range[0]
                        item_aux = [view_idx, range]
                        item = {view_idx: range}
...
def view_index_consistency(unstruct_det):
    '''
            ...
            if len(detection) == 1:
                if not indexed_det_list:
                    ...
                    if None not in indexed_det_list[prev_idx]:
                        ...
                        if indexed_det_list[prev_idx][single_idx]['idx'] == detection[0]['idx']:
                            idx_frame_keypoint_list[single_idx] = detection[0]
                        elif calc_keypoint_overlap(indexed_det_list[prev_idx][single_idx]['keypoints'],
                                                   detection[0]['keypoints']) <= 15:
                            idx_frame_keypoint_list[single_idx] = detection[0]
                        else:
                ...
                if not indexed_det_list:
                    ...
                    if unidentified_detections:
                        if len(unidentified_detections) == 2:
                            # check keypoint overlap
                            for unident in unidentified_detections:
                                match_id = identify_keypoint_index([unident], indexed_det_list[prev_idx])
                            ...
                            none_idx = idx_frame_keypoint_list.index(None)
                            idx_frame_keypoint_list[none_idx] = unidentified_detections[0]
                            
                assert None not in idx_frame_keypoint_list
                
        #assert None not in idx_frame_keypoint_list
        #print(i, idx_frame_keypoint_list)
        assert len(idx_frame_keypoint_list) == 2, f'Error'
...
def get_missing_det_list(coorect_missing_key):
    ...
    for person_id in range(2):
        ...
        for view in range(len(coorect_missing_key)):
            missing_frame_list = get_missing_frames(coorect_missing_key[view], person_id)
            pers_miss_range = []
    ...
    for view_list in box_list:
        for frame_box in view_list:
                for idx, box in enumerate(frame_box):
                    ...
                    box_perimeter[idx] += (mask_h + mask_w)
...
def main_json_processor(all_detections):
    ...
    for id_v, det_view in enumerate(corrected_key_view_list):
        ice_keypoints_one.append(view_index_consistency(det_view))
    ...
    return interpool_keypoints, missing_keypoints



def first_last_guarantee(keypoint_list):
    #Ensure that first frame has both detections
    n_people = 2
    initial_frame = 0
    last_frame = -1
    search_initial = True
    ...
    while search_initial:
        ...
        for view_id in range(len(keypoint_list)):
            ...
            view_det_len_ini.append(len(keypoint_list[view_id][last_frame]))
        if len(set(view_det_len_ini)) == 1 and set(view_det_len_ini) == {n_people}:
    ...
    while search_final:
        view_det_len_final = []
        for view_id in range(len(keypoint_list)):
            view_det_len_final.append(len(keypoint_list[view_id][last_frame]))
            view_det_len_final.append(len(keypoint_list[view_id][last_frame]))
        if len(set(view_det_len_final)) == 1 and set(view_det_len_final) == {n_people}:
            search_final = False
        else:
            last_frame -= 1
